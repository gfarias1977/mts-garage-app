...
interface CheckListGaragePaginationProps {
  page: number;
  pageSize: number;
  total: number;
  totalPages: number;
...
export function CheckListGaragePagination({
  page,
  pageSize,
  total,
  totalPages,
  searchParams,
  navigate,
}: CheckListGaragePaginationProps) {
  const { t } = useI18n();
    ...
    <div className="flex items-center justify-between gap-4 text-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex gap-1">
          ...
          <Button size="sm" variant="outline" disabled={page >= totalPages} onClick={() => go(totalPages)}>
            {t('table.last')}
          </Button>
        </div>
      </div>
