...
import { getSparePartsByWorkOrderServicePagedAction } from '@/app/(dashboard)/work-orders/actions';
import type { GetSparePartsResult, SparePartSortColumn } from '@/data/workOrderSpareParts';
import { WorkOrderServiceSparePartEditSheet } from './WorkOrderServiceSparePartEditSheet';
...
interface WorkOrderServiceSparePartsModalProps {
  open: boolean;
  workOrderId: string;
...
export function WorkOrderServiceSparePartsModal({
  open,
  workOrderId,
  workOrderServiceId,
  serviceName,
  onClose,
}: WorkOrderServiceSparePartsModalProps) {
  const { t } = useI18n();
  ...
  const loadData = useCallback(() => {
    setLoading(true);
    getSparePartsByWorkOrderServicePagedAction({
      workOrderServiceId,
      page,
      pageSize,
      search,
  ...
  }, [workOrderServiceId, page, pageSize, search, sortBy, sortDir]);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {t('work_orders.spare_parts.title')}
            ...
            <TableHeader>
              <TableRow>
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort('id')}>
                  {t('work_orders.spare_parts.col.id')}<SortIcon col="id" />
                </TableHead>
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort('description')}>
                  {t('work_orders.spare_parts.col.description')}<SortIcon col="description" />
                </TableHead>
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort('cost')}>
                  {t('work_orders.spare_parts.col.cost')}<SortIcon col="cost" />
                </TableHead>
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort('status')}>
                  {t('work_orders.spare_parts.col.status')}<SortIcon col="status" />
                </TableHead>
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort('createdAt')}>
                  {t('work_orders.spare_parts.col.created_at')}<SortIcon col="createdAt" />
                </TableHead>
                <TableHead className="cursor-pointer select-none" onClick={() => handleSort('createdBy')}>
                  {t('work_orders.spare_parts.col.created_by')}<SortIcon col="createdBy" />
                </TableHead>
      ...
      <WorkOrderServiceSparePartEditSheet
