...
interface ServicePricesPaginationProps {
  page: number;
  pageSize: number;
  total: number;
  totalPages: number;
...
export function ServicePricesPagination({
  page,
  pageSize,
  total,
  totalPages,
  ...
  searchParams,
  navigate,
}: ServicePricesPaginationProps) {
  const { t } = useI18n();

  function go(newPage: number) {
    navigate(buildUrl(searchParams, serviceId, { page: String(newPage) }));
  }

  function handlePageSize(value: string | null) {
    if (!value) return;
    navigate(buildUrl(searchParams, serviceId, { pageSize: value, page: '1' }));
  }
