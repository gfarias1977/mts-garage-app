...
import { getServices, type SortColumn } from '@/data/services';
import { ServicesModule } from '@/components/services/ServicesModule';

const VALID_SORT_COLUMNS: SortColumn[] = [
  'id', 'categoryName', 'name', 'description', 'createdAt', 'createdBy', 'status',
];
...
export default async function ServicesPage({ searchParams }: { searchParams: SP }) {
  ...
  const result = await getServices({ userId: user.id, search, sortBy, sortDir, page, pageSize });
