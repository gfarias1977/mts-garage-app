...
import { WorkOrdersMechanicRowActions } from './WorkOrdersMechanicRowActions';
import { buildUrl } from '@/components/work-orders/urlHelpers';

type ActionType = 'edit' | 'observations' | 'diagnosis';

interface WorkOrdersMechanicTableProps {
  rows: WorkOrderRow[];
  sortBy: SortColumn;
  sortDir: 'asc' | 'desc';
...
export function WorkOrdersMechanicTable({
  rows,
  sortBy,
  sortDir,
  searchParams,
  onAction,
}: WorkOrdersMechanicTableProps) {
  const { t } = useI18n();
